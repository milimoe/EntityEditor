    ...
    public partial class ShowDetail : Form
    {
        ...
        private void 加技能_Click(object sender, EventArgs e)
        {
            if (NowClick == 0 && SkillManager != null && BaseEntity is Character c)
            {
                ...
                {
                    ShowList l = new();
                    l.AddListItem(SkillManager.LoadedSkills.OrderBy(kv => kv.Value.Id).Where(kv => kv.Value.SkillType != FunGame.Core.Library.Constant.SkillType.Item).Select(kv => EntityEditor.GetSkillDisplayName(SkillManager, kv.Key)).ToArray());
                    l.ShowDialog();
                    string selected = l.SelectItem;
